    ...
    useEffect(() => {
      const fetchAditional = (cartItems) => {
        return cartItems.map( (item) => {
          ...
          return Promise.all([proposalInfo,categoriesPath])
            .then( ([proposalInfo,categoriesPath]) => {
              let categoriesPathString = ""
              for (let i = 0; i < categoriesPath.length; i++) {
                if (i == categoriesPath.length - 1)
                  categoriesPathString = categoriesPathString.concat(categoriesPath[i])
                else
                  categoriesPathString = categoriesPathString.concat(categoriesPath[i]).concat(" - ")
              }
              return { ...item, ...proposalInfo, category: categoriesPathString,...missingTestFields }
            } )
      ...
      if (!cartContext.loading ) {
        Promise.all(fetchAditional(cartContext.cart))
          .then( (items) => {
            setBasket(items)
            setLoading(false)
          })
