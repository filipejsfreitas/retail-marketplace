...
const SellerInfo = ({idSeller}) => {
  const { data: seller, loading } = useFetchData(`${process.env.NEXT_PUBLIC_HOST}/seller/${idSeller}`)
...
const GetProposals = ({proposals}) =>{
  const info = []
   proposals.map(prop => {
    const { data: product, loading } = useFetchData(`${process.env.NEXT_PUBLIC_HOST}/product/${prop.product_id}`)
...
export default function SellerPage({categories}){
  ...
  const { data: proposals, loading } =
    useFetchData(`${process.env.NEXT_PUBLIC_HOST}/proposal/seller/${idSeller}`, { default: {} })
