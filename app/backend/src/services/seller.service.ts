...
export class SellerService{
    ...
    public async makeComment(comment: SellerCommentDto, client_id: string, seller_id: string): Promise<SellerComment>{
        ...
        var newNumberScores = findSeller.numberRating + 1;

        var newScore = (findSeller.rating * findSeller.numberRating + comment.rating) / newNumberScores; 

        const updateSeller: Seller = await this.sellers.findOneAndUpdate({_id: seller_id},{rating:newScore,numberRating: newNumberScores}, { new: true });
    ...
    public async deleteComment(comment_id: string, client_id: string): Promise<SellerComment> {
        ...
        var newNumberScores = seller.numberRating - 1;
        ...
        const sellerUpdated : Seller = await this.sellers.findOneAndUpdate({seller_id: seller._id},{rating:newScore,numberRating: newNumberScores },{ new: true });
    ...
    public async updateComment(comment_id: string, client_id: string, new_comment: SellerCommentDto){
        ...
        var newScore = (seller.rating * seller.numberRating - findComment.rating + new_comment.rating) /  seller.numberRating; 

        const sellerUpdated : Seller = await this.sellers.findOneAndUpdate({seller_id: seller._id},{rating:newScore },{ new: true });

        const comment_updated : SellerComment = await this.comments.findOneAndUpdate({_id: comment_id}, {...new_comment, date: new Date()}, {new: true});
