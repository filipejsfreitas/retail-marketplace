...
export class ClientService {
    ...
    public async createClient(client : CreateClientDto): Promise<Client> {
        const clientCreated = await this.clients.create({firstName: client.firstName, lastName: client.lastName,
                                                    phoneNumber: client.phoneNumber, favoriteProducts: [], client_id: client.client_id
                                                    });
    ...
    public async updateClient(client : UpdateClientDto, client_id): Promise<Client> {
        const clientUpdated= await this.clients.findOneAndUpdate({client_id: client_id},{firstName: client.firstName, lastName: client.lastName,
                                                    phoneNumber: client.phoneNumber}, {new:true});
    ...
    public async deleteFromFavorites(client_id: string, product_id: string): Promise<Client>{
        ...
        var index= client.favoriteProducts.indexOf(product_id);
